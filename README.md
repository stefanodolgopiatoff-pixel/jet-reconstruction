...
The project uses a Convolutional Neural Network model, designed with multiple Conv2D layers for feature extraction, MaxPooling2D for downsampling, and Dropout layers for regularization, culminating in Dense layers for classification with a softmax activation. The model is compiled using the Adam optimizer and categorical_crossentropy loss.
